...
| 1 | Трёхконтактный запираемый разъём с пигтейлом | Припаивается к `G3`, `G4`, `GND` протоплаты; соединяет жгут кнопок с контроллером. |
...
В этой конфигурации Grove-порт полностью занят Unit CAN: `G2/GPIO2` - CAN TX, `G1/GPIO1` - CAN RX. Кнопки подключаются к подписанным площадкам протоплаты AtomS3R-Ext: `G3`, `G4` и `GND`. Входы используют внутреннюю подтяжку к питанию; при нажатии кнопка замыкает вход на общий провод FTU/GND.
